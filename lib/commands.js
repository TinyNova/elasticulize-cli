...
function Commands (paths) {
    ...
    this._init = () => {
        return new Promise(async (res, rej) => {
            ...
            if (this.config.awsConfig) {
                this.config.connectionClass = require('https-aws-es');
            }
    ...
    this['cluster:migrate'] = async function () {
        ...
        const versionsResponse = await getVersions(this.client);
    ...
    this['cluster:migrate:create'] = async function ({ name }) {
        name = name || 'migration';
        const fileName = `${moment().format('YYYYMDDhhmmss')}-${name}.js`;
